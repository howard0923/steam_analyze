...
save(steam,file="data/Prep_steam.rdata")
#########################################
##########################################
##導入資料
load("data/Prep_steam.rdata")
steam$owners <- as.factor(steam$owners)
########篩選需要欄位
...
# 從category中找出較重要變數
GameSale_gen <- rpart(owners ~ isIndie + isAction + isCasual + isAdventure + isStrategy 
                      + isSimulation + isRPG + isEarlyAccess + isSports + isRacing + isViolent + isGore 
                      + isNudity + isSexualContent, data = train_set, control = rpart.control(cp = 0.001))
summary(GameSale_gen) # 在genre中，沒有較重要變數

# 刪除資料中多餘的變數
train_set_df = train_set[,c(3, 17, 19, 20, 23, 24, 27,)]
test_set_df = test_set[,c(3, 17, 19, 20, 23, 24, 27,)]
